...
export function GoogleButton({
  ...
  const isActive = isLoading || loading;
    ...
    >
      <span className="inline-flex items-center justify-center gap-2.5">
        {isActive ? (
          ...
          <svg
            className="h-5 w-5 shrink-0 transition-transform duration-200 group-hover:scale-[1.03]"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
        ...
        {isActive ? "Connecting to Google..." : children}
      </span>
    </Button>
  );
}
