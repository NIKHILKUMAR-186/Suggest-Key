...
function AdminBookingQueue() {
  ...
  async function assign(reqId: string, mentorId: string) {
    ...
    try {
      ...
      if (!req) throw new Error("Booking not found");
       ...
       const json = contentType.includes("application/json") ? await res.json() : { error: `HTTP ${res.status}` };
  ...
  async function autoMatch(reqId: string) {
    ...
    try {
      ...
      const autoContentType = res.headers.get("content-type") ?? "";
      const json = autoContentType.includes("application/json") ? await res.json() : { error: `HTTP ${res.status}` };
      if (!res.ok) throw new Error(json?.message || json?.error || `auto-match failed (${res.status})`);
