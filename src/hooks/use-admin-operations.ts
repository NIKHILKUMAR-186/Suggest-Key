...
export function useRescheduleBooking() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (!res.ok) throw new Error(json?.error || `Reschedule failed (${res.status})`);
      return json;
    },
    onSuccess: () => {
...
export function useMarkNoShow() {
  ...
  return useMutation({
    mutationFn: async (payload: { bookingId: string; actor: "student" | "mentor"; notes?: string }) => {
      ...
      const json = await safeJson(res);
      if (!res.ok) throw new Error(json?.error || `No-show failed (${res.status})`);
      return json;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-session-requests"] });
...
export function useAutoMatch() {
  ...
  return useMutation({
    mutationFn: async (payload: { bookingId: string }) => {
      ...
      const json = await safeJson(res);
      if (!res.ok) throw new Error(json?.error || `Auto-match failed (${res.status})`);
      return json;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-session-requests"] });
