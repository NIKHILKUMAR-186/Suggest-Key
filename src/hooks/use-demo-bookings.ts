...
async function safeJson(res: Response): Promise<any> {
  ...
  return { success: false, error: `Server returned ${res.status} ${res.statusText}: ${text.substring(0, 200)}` };
}
...
export function useAssignMentorToDemo() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const { data, error } = await supabase.rpc("assign_demo_mentor", {
        p_booking_id: payload.bookingId,
        p_mentor_id: payload.mentorId,
        p_client_version: payload.clientVersion ?? 1,
      });
      if (error) throw new Error(error.message);
      const result = Array.isArray(data) ? data[0] : data;
      if (!result?.success) throw new Error(result?.error || "Unable to assign mentor");
      return result;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-demo-bookings"] });
...
export function useAdminTakeDemoSession() {
  ...
  return useMutation({
    mutationFn: async (payload: { bookingId: string; clientVersion?: number }) => {
      const { data, error } = await supabase.rpc("admin_take_demo_session", {
        p_booking_id: payload.bookingId,
        p_client_version: payload.clientVersion ?? 1,
      });
      if (error) throw new Error(error.message);
      const result = Array.isArray(data) ? data[0] : data;
      if (!result?.success) throw new Error(result?.error || "Unable to take session");
      return result;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-demo-bookings"] });
...
export function useMentorRespondDemoAssignment() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (!res.ok) throw new Error(json?.error || `response failed (${res.status})`);
      return json;
    },
    onSuccess: (data, variables) => {
