...
export async function addDemoMeetingLink(
  ...
  if (!res.ok || !json.success) {
    throw new Error(json?.error || `Failed to add meeting link (${res.status})`);
  }
  return json;
}
...
export async function expireDemoAssignments(): Promise<{ expired_count: number }> {
  ...
  const json = await safeJson(res);
  if (!res.ok || !json.success) {
    throw new Error(json?.error || `Failed to expire assignments (${res.status})`);
  }
  return json;
}
...
export async function assignMentorToDemo(
  ...
  const json = await safeJson(res);
  if (!res.ok || !json.success) {
    throw new Error(json?.error || `Failed to assign mentor (${res.status})`);
  }
  return json;
}
...
export async function adminTakeDemoSession(
  ...
  const json = await safeJson(res);
  if (!res.ok || !json.success) {
    throw new Error(json?.error || `Failed to take session (${res.status})`);
  }
  return json;
}
